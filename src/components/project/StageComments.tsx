...
const StageComments = ({ stageId, stageName, projectId, projectName }: Props) => {
  ...
  const sendComment = async () => {
    ...
    // Notify the other party using central dispatcher (App + Email + WhatsApp per user prefs)
    try {
      ...
      if (project) {
        ...
        if (isOwner) {
          ...
          if (invite?.architect_user_id) {
            await notify({
              event: 'stage_comment',
              userId: invite.architect_user_id,
              projectId,
          ...
          await notify({
            ...
            message: `Arq. ${profile?.full_name || 'Arquiteta'} deixou um comentário na etapa "${stageName}" do projeto "${projectName}".`,
            ctaUrl: `${window.location.origin}/projeto/${projectId}`,
          });
        }
      }
